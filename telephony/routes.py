...
async def transfer_fallback(request: Request) -> Response:
    """Twilio hits this via <Dial action="..."> when the rep leg of a
    transfer doesn't complete normally - no answer, busy, failed, or the
    rep's own hangup before talking. Without this, an unanswered transfer
    leaves the caller listening to ringback until Twilio's own hard call
    timeout, with no graceful message and no record of what happened.

    DialCallStatus values: completed, busy, no-answer, failed, canceled.
    "completed" means the rep answered and the call already happened (or
    the rep hung up after talking) - in that case we say nothing further
    and just let the call end normally.
    """
    if not _check_webhook_secret(request.path_params):
        return Response(status_code=404)
    ...
    if _twilio_validator:
        ...
        signature = request.headers.get("X-Twilio-Signature", "")
        if not _twilio_validator.validate(url, params, signature):
            logger.warning("[TELEPHONY] Invalid Twilio signature on /transfer-fallback - rejecting")
            return Response(status_code=404)

    call_sid = params.get("CallSid", "unknown")
    dial_status = params.get("DialCallStatus", "unknown")
    logger.info(f"[TELEPHONY] Transfer fallback - CallSid={call_sid} DialCallStatus={dial_status}")

    if dial_status == "completed":
        # Rep answered and the call ran its course - nothing to do.
        twiml = """<?xml version="1.0" encoding="UTF-8"?>
<Response><Hangup/></Response>"""
    else:
        # Rep didn't answer (no-answer/busy/failed/canceled). Tell the
        # caller plainly rather than just dropping them.
        twiml = """<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Say>Sorry, our team isn't available right now. We'll follow up with you shortly. Thanks for your time.</Say>
    <Hangup/>
</Response>"""
